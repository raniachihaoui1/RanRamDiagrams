"""Shared utilities for the technical_diagrams captioning tool.

Self-contained copy — does NOT depend on the repo-level scripts/utils.py.
Logging and the idempotency marker file live inside this folder.
"""
...
def get_logger(name: str = "technical_diagrams") -> logging.Logger:
...
def log_processed(path: Path) -> None:
    """Append a processed-file marker to the log (load-bearing idempotency state)."""
    with LOG_FILE.open("a", encoding="utf-8") as fh:
        fh.write(f"PROCESSED {path.resolve()}\n")


def already_processed(path: Path) -> bool:
    """Return True if this file already has a PROCESSED entry in the log."""
